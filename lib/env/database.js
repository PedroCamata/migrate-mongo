...
module.exports = {
  async connect() {
    ...
    db.collection = function (name, options) {
      ...
      // Only wrap if migration up and autoRollback is enabled
      if (db.isMigrationDown || !db.autoRollbackEnabled) {
        return collection;
      }
    ...
    db.autoRollback = async function () {
      ...
      const rollbackHandlers = {
        ...
        insertMany: (targetCollection, params) => {
          return targetCollection.insertMany(params)
        },
