...
export default async (db) => {
  ...
  const changelog = await changelogCollection.find({}).sort({ appliedAt: 1 }).toArray();

  // Changelog sequence is the source of truth in case file names don't reflect the run order
  const useFileHashTest = useFileHash === true;
  const statusTable = await Promise.all(changelog
